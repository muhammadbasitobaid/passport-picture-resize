...
- **The editor** lives in `src/components/editor/` and uses the "PhotoFlow" design (3-column layout: photo strip · stage+controls · document picker/export). `editor.tsx` is the client orchestrator holding all state (the `photos[]` array + `activeId`, preset, crop, adjustments, bg toggle/color, copies); sub-components (`photo-strip`, `stage`, `control-bar`, `document-picker`, shared `types.ts`) are presentational and receive state + callbacks. **Multi-photo:** several uploads live in the strip; crop/adjustments are global and apply to the active photo, while each photo caches its own removed-background `foreground`. The page (`src/app/page.tsx`) renders `<Editor />` full-screen (no `container`).
